...
Future<List<Block>> getBlockOfSubCategory(String pageID) async {
  ...
  final publishedBlocksSnapshot = await publishedBlocksRef.where("sub_cat_id" ,isEqualTo: pageID).get();

  List<Block> blocks = [];

  publishedBlocksSnapshot.docs.forEach((doc) {
  ...
  print("Sub category blocks ${blocks.length}");
  return blocks;
}
Future<List<Block>> getBlockOfCategory(String pageID) async {
  final publishedBlocksRef = FirebaseFirestore.instance.collection('published_blocks');
  final publishedBlocksSnapshot = await publishedBlocksRef.where("page_id" ,isEqualTo: pageID).where("sub_cat_id", isEqualTo: null).get();
...
Future<List<Block>> getHomePageBlocks() async {
  ...
  final publishedBlocksRef = FirebaseFirestore.instance.collection('published_blocks');
  final publishedBlocksSnapshot = await publishedBlocksRef.where("page_id" ,isEqualTo: trackIds[0]).orderBy("sequence").get();
  ...
  await trackBlockCollection.where("block_id",isEqualTo: blockId).orderBy("sequence").get();
  ...
  if(trackId != "") {
    ...
    try {
      ...
      if (doc.exists) {
        ...
        await recentlyPlayedRef.update(
      ...
      print("Error adding track to recently played collection: $error");
    }
  }
}
...
Future<void> incrementPlayCount(String trackId) async {
  print("document id $trackId");
  if(trackId != "") {
    try {
      final trackRef = FirebaseFirestore.instance.collection('Tracks').doc(
          trackId);
      ...
      print('Play count incremented for track $trackId');
    } catch (e) {
      print('Failed to increment play count for track $trackId: $e');
    }
  }
}
...
//   } catch (error) {
//     print("Error getting recently played tracks: $error");
//     return [];
//   }
// }
...
Future<List<AudioTrack>> getRecentlyPlayedTracks() async {
  ...
  final recentlyPlayedRef = _firestore
      ...
      .collection("Tracks");
  // Query the "recently_played" collection to get the list of recently played track IDs
  final querySnapshot1 = await recentlyPlayedRef.orderBy("datetime", descending: true).get();
  ...
  for (int i = 0; i < trackIds.length; i += 10) {
    final batchIds = trackIds.sublist(i, i + 10 > trackIds.length ? trackIds.length : i + 10);
...
Future<List<AudioTrack>> getRecentlyPlayedTracksByCategory( String category) async {
  ...
  // Query the "recently_played" collection to get the list of recently played track IDs
  final querySnapshot1 = await recentlyPlayedRef.orderBy("datetime", descending: true).get();
  //print(querySnapshot1.docs[0].id);
  // Create a list of track IDs from the query snapshot
  final List<String> trackIds = querySnapshot1.docs.map((doc) => doc.id).toList();
 // print(trackIds);
...
Future<List<CategoryBlock>> getCategoryBlocks() async {
  ...
  final querySnapshot = await tracksRef
      .where('sequence', isGreaterThan: 1)
      .orderBy("sequence")
      .get();
  ...
  print("category length ${featuredTracks.length}");
  return featuredTracks;
}
...
// Create a list of track IDs from the query snapshot
  ...
  for (final doc in snapshots.docs) {
   ...
   // final tracksRef = do.get();
    ...
    final tracksQuerySnapshot = await tracksCollection.where(FieldPath.documentId, whereIn: tracks).get();
...
Future<List<Collection>> fetchCollectionsBySubcategory(String category) async {
  final collectionRef = FirebaseFirestore.instance.collection('collections').where('sub_category', isEqualTo: category).limit(3);
  final snapshots = await collectionRef.get();
...
Future<List<Collection>> getTopCollections(String category) async {
  final collectionRef = FirebaseFirestore.instance.collection('collections').where('category', isEqualTo: category).limit(6);
  final snapshots = await collectionRef.get();
...
Future<bool> updateUserEmailAndData(String newEmail, String newName, String password) async {
  ...
  if (user != null) {
    try {
      ...
      print('User email and data updated successfully.');
      return true;
...
Future<void> fetchCategories() async {
  try {
    final categoriesCollection = FirebaseFirestore.instance.collection('published_categories');
    final categoriesSnapshot = await categoriesCollection.orderBy('sequence',descending: false).get();
...
Future<List<SubCategory>> fetchSubcategoriesBlocks(String categoryID) async {
  ...
  List<SubCategory> subcategories = querySnapshot.docs
      .map((document) => SubCategory.fromFirestore(document))
      .toList();
