...
Future<List<bool>> getSelectedDays() async {
  SharedPreferences prefs = await SharedPreferences.getInstance();
  List<String> selectedDaysStrList = prefs.getStringList('selectedDays') ?? List.filled(7, 'false');
...
Future<List<String>> getFavoriteListFromSharedPref() async {
  ...
  SharedPreferences preferences = await SharedPreferences.getInstance();
  return await preferences.getStringList("fav_list")??[];
}

Future<void> removeFromFavoritesList(String trackId) async {
  SharedPreferences preferences = await SharedPreferences.getInstance();
  List<String> favList =  await preferences.getStringList("fav_list")??[];
...
Future<List<String>> getFavoritePlayListFromSharedPref() async {
  ...
  SharedPreferences preferences = await SharedPreferences.getInstance();
  return await preferences.getStringList("fav_play_list")??[];
}

Future<void> removeFromFavoritesPlayList(String blockId) async {
  SharedPreferences preferences = await SharedPreferences.getInstance();
  List<String> favList =  await preferences.getStringList("fav_play_list")??[];
...
showLoaderDialog(BuildContext context,String message){
  WillPopScope alert = WillPopScope(child: AlertDialog(
    ...
    content: Row(
      children: [
        ...
        Expanded(flex:1,child:Container(margin: const EdgeInsets.only(left: 7),child:Text(message))),
      ],),
  ),
...
playTrack(AudioTrack audioTrack){
  ...
  getIt<PageManager>().playSinglePlaylist(MediaItem(
    ...
    extras: {'url': audioTrack.trackUrl,'id':audioTrack.trackId,'categories':audioTrack.categories[0].categoryName},
  ),audioTrack.trackId);
...
Future<void> saveCategories(List<Categories> categories) async {
  ...
  List<String> categoryStrings = categories.map((category) => jsonEncode(category.toMap())).toList();
...
class SlideFromBottomPageRoute extends PageRouteBuilder {
  ...
  SlideFromBottomPageRoute({required this.page})
      ...
      transitionsBuilder: (_, animation, __, child) =>
          SlideTransition(position: Tween<Offset>(begin: Offset(0, 1), end: Offset.zero).animate(animation), child: child));
}
