...
import 'package:sleeptales/widgets/series_track_image_widget.dart';
import 'package:sleeptales/widgets/shimmerwidgets/shimmer_series_track_image_widget.dart';
...
class PlayListTracksScreen extends StatefulWidget {
  ...
  PlayListTracksScreen({super.key,required this.panelFunction,required this.block});
...
class _TrackListScreenState extends State<PlayListTracksScreen> {
  ...
  void _onPlaylistChanged() {
    setState(() {
      ...
      currentPlaylistIsPlaying = currentPlayingList.every((id) => tracksList.any((track) => track.trackId == id));
    });
  }
    ...
    if (favoritesDocSnapshot.exists) {
      ...
      if(favoritesData!['playlist'] != null) {
        ...
        if (!favoritesDataPlayList.contains(playListId)) {
          ...
          await userFavoritesDocRef.update(
              {'playlist': favoritesDataPlayList});
        }
      }else{
        await userFavoritesDocRef.set({'playlist': [playListId]});
      }
    } else {
      await userFavoritesDocRef.set({'playlist': [playListId]});
    }
            ...
            SliverAppBar(
              ...
              flexibleSpace: FlexibleSpaceBar(
                  ...
                  background: CachedNetworkImage(
                    ...
                    fit: BoxFit.cover,
                    placeholder:(context, url) =>  Image.asset("images/placeholder_image.jpg",fit: BoxFit.cover,),
                  )
              ...
              delegate: SliverChildListDelegate([
                Padding(
                  ...
                  Row(
                    ...
                    children: [
                      Expanded(child: Text(widget.block.title,style: TextStyle(fontWeight: FontWeight.bold,fontSize: 24.sp),)),
                      ...
                      Stack(
                        children: [
                          ...
                          }, icon:isPlaylistFavorited(widget.block.id)?Icon(Icons.favorite,size: 24.w,):Icon(Icons.favorite_border,size: 24.w,),constraints: BoxConstraints(),)
                        ],
                      )
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      Column(
                        ...
                        children: [
                          Padding(padding: EdgeInsets.only(left: 0.w),
                            child:
                            Text(
                              widget.block.author,
                              style: TextStyle(
                ...
                Padding(padding: EdgeInsets.symmetric(horizontal: 16.w),
                  child:CustomButton(title: (currentPlaylistIsPlaying && playing)?"Pause":"Play", onPress: (){
                  ...
                  }, color: tracksList.isEmpty?Colors.grey:Colors.white, textColor:tracksList.isEmpty?Colors.black12:textColor),
                ),
                SizedBox(height: 16.h,),
                Padding(padding: EdgeInsets.symmetric(horizontal: 16.w),
                  child:Text(widget.block.description,style: TextStyle(fontSize: 16.sp),),
                ),

                if(trackListLoading || tracksList.isNotEmpty)
                Padding(padding: EdgeInsets.only(top: 32.h,left: 16.w,right: 16.w),
                  child:  Divider(
                    height: 1.0,
                    color: Colors.grey,
                  ),
                ...
                ListView.separated(
                  ...
                  itemBuilder: (BuildContext context, int index) {
                    return Padding(padding: EdgeInsets.fromLTRB(0, 8.h, 0, 8.h),
                      child: widget.block.showSeriesImg != "1"?

                      SeriesTrackListWidget(audioTrack:tracksList[index],tap: (){
                        getIt<PageManager>().loadPlaylist(tracksList,index);
                        widget.panelFunction(false);
                      },onTapPlayPause: (){
                        if(currentPlaylistIsPlaying && currentMediaItem.id == tracksList[index].trackId && playing){
                          getIt<PageManager>().pause();
                        }else{
                          getIt<PageManager>().loadPlaylist(tracksList,index);
                          getIt<PageManager>().play();
                        ...
                        currentPlaying: (currentPlaylistIsPlaying && currentMediaItem.id == tracksList[index].trackId && playing)?true:false,
                      ):


                      SeriesTrackListImageWidget(audioTrack:tracksList[index],tap: (){
                        getIt<PageManager>().loadPlaylist(tracksList,index);
                        widget.panelFunction(false);
...
Widget _buildShimmerListViewImage(String type) {
  return     ListView.separated(
    ...
    itemBuilder: (BuildContext context, int index) {
      return Padding(padding: EdgeInsets.fromLTRB(0, 8.h, 0, 8.h),
        child: type == "1"?ShimmerSeriesTrackListImageWidget():ShimmerSeriesTrackListWidget()
