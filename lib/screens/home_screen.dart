...
import 'package:sleeptales/screens/explore_screen.dart';
import 'package:sleeptales/screens/forme_screen.dart';
import 'package:sleeptales/screens/profile_settings_screen.dart';
...
class HomeScreenState extends State<HomeScreen> {
  ...
  MediaItem? item;
  ScrollControllerHelper _scrollControllerHelper = ScrollControllerHelper();
  ...
  void showPanel(bool dontShowPanel) {
    print("panel function parameter $dontShowPanel");
  ...
  Widget build(BuildContext context) {
    ...
    MaterialPageRoute(builder: (_) => ForMeScreen(showPanel,_scrollControllerHelper,openExploreTab)),
    ),
    ),
    Offstage(
    ...
    onGenerateRoute: (routeSettings) =>
    MaterialPageRoute(builder: (_) => ProfileSettingsScreen(showPanel)),
    ),
    ),
            ...
            minHeight: defaultTargetPlatform == TargetPlatform.android?(kBottomNavigationBarHeight+78.h):MediaQuery.of(context).padding.bottom+kBottomNavigationBarHeight+78.h,
            controller: panelController,
              panel:
                  Stack(
                    children: [
                      MusicPlayerScreen(playList: playlist.length>1?true:false,panelControllerNest: panelController,),
                                if(panelController.isAttached)
                                          ...
                                          child:
                                           Row(children: [
                                                   ValueListenableBuilder(
                                                     valueListenable: getIt<PageManager>()
                                                         .currentMediaItemNotifier,
                                                     builder: (BuildContext context,
                                                         MediaItem mediaItem, Widget? child) {
                                                       return Container(
                                                         child: ClipRRect(
                                                           borderRadius: BorderRadius.only(
                                                               topLeft: Radius.circular(16)),
                                                     ...
                                                     children: [
                                                       ...
                                                       ValueListenableBuilder(
                                                         ...
                                                         builder: (BuildContext context,
                                                           ...
                                                           return Container(
                                                               child: Text(
                                                                 progress.current.inMinutes
                                                                     .remainder(60)
                                                                     .toString()
                                                                     .padLeft(2, '0') +
                                                                     ":" +
                                                                     progress.current.inSeconds
                                                                         .remainder(60)
                                                                         .toString()
                                                                         .padLeft(2, '0'),
                                                                 style: TextStyle(
                                           ...
                                           if(item?.extras?["categories"] != "Soundscape")
          ...
          Positioned(
            ...
            duration: Duration(milliseconds: 300),
              child:
                Container(
                  height: defaultTargetPlatform == TargetPlatform.android ?kBottomNavigationBarHeight+2.h:(MediaQuery.of(context).padding.bottom+kBottomNavigationBarHeight+2.h),
...
class PlayButtonNew extends StatelessWidget {
  const PlayButtonNew({Key? key}) : super(key: key);
  @override
  Widget build(BuildContext context) {
...
class AudioProgressBarHome extends StatelessWidget {
  const AudioProgressBarHome({Key? key}) : super(key: key);
  @override
  Widget build(BuildContext context) {
    final pageManager = getIt<PageManager>();
...
class CurrentSongTitleSmall extends StatelessWidget {
  const CurrentSongTitleSmall({Key? key}) : super(key: key);
  @override
  Widget build(BuildContext context) {
    final pageManager = getIt<PageManager>();
